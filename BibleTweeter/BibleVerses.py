...
books = (("Genesis",(31,25,24,26,32,22,24,22,29,32,32,20,18,24,21,16,27,33,38,18,34,24,20,67,34,35,46,22,35,43,55,32,20,31,29,43,36,30,23,23,57,38,34,34,28,34,31,22,33,26)), 
...
def convert_absolute(value):
	...
	for book in books[:]:
		for chapter in book[1]:
			if currentVerse + chapter >= value:
				verseNumber = value - currentVerse
				break
			else:
				chapterNumber+=1
	...
	return books[bookNumber][0] + "+" + str(chapterNumber) + ":" + str(verseNumber) 

def get_verse(location):
	r = requests.get('https://bible-api.com/' + location)
